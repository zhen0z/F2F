function rrule(S::jutulModeling{D, T}, Transmissibilities::AbstractVector{T}, ϕ::AbstractVector{T}, f::Union{jutulForce{D, N}, jutulVWell{D, N}};
    ...
    function pullback(dy)
        ...
        n_faces = length(Transmissibilities)
        n_cells = prod(S.model.n)
        dTransmissibilities = g[1:n_faces]
        dϕ = g[n_faces + 1 : n_faces + n_cells] * prod(S.model.d)
        return NoTangent(), dTransmissibilities, dϕ, NoTangent()
    end
    return output, pullback
end

function rrule(S::jutulModeling{D, T}, Transmissibilities::AbstractVector{T}, ϕ::AbstractVector{T}, f::jutulSource{D, N};
    state0=nothing, visCO2::T=T(visCO2), visH2O::T=T(visH2O),
    ρCO2::T=T(ρCO2), ρH2O::T=T(ρH2O), info_level::Int64=-1) where {D, T, N}
